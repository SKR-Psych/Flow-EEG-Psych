...
class FBCSPExtractor(BaseEstimator, TransformerMixin):
    ...
    def fit(self, X, y):
        for band_name, (fmin, fmax) in self.frequency_bands.items():
            ...
            X_filtered = mne.filter.filter_data(X.astype(np.float64), self.sfreq, l_freq=fmin, h_freq=fmax, verbose=False)
...
def main():
    ...
    # Fit FBCSP on the collected data
    print("Fitting FBCSP on collected data...")
